...
for width in widths:
    ...
    if mean_eigenvalue_spectra and all(len(eig_list) == len(mean_eigenvalue_spectra[0]) for eig_list in mean_eigenvalue_spectra):
        ...
        early_time_limit = 100 # Example: Focus on the first 5 training time units
...
for filename in os.listdir(data_dir):
    if filename.endswith(".pkl"):
        ...
        if width is None or mean_eigenvalue_spectra_times is None or ntk_record_times_eigenvalues is None:
            print(f"Skipping {filename}: Missing required data.")
            continue

        print(f"Processing eigenvalue data for width: {width}")

        if not ntk_record_times_eigenvalues or not mean_eigenvalue_spectra_times:
            print(f"No eigenvalue data recorded for width {width}. Skipping plotting.")
            continue

        # Find the index corresponding to the initial time (or the first recorded time)
        initial_time_index = 0
        # Assuming the times are sorted, the first element is the initial time or closest to it.
        ...
        # Create a figure for this width
        pl.figure(figsize=(12, 8))

        # Iterate through each eigenvalue
        num_eigenvalues = len(initial_eigenvalue_spectrum)
        for eigenvalue_index in range(num_eigenvalues):
            f_lambda_t_values = []
            training_times_for_plot = []

            # Iterate through each recorded time
            for time_idx, t in enumerate(ntk_record_times_eigenvalues):
                if time_idx < len(mean_eigenvalue_spectra_times) and eigenvalue_index < len(mean_eigenvalue_spectra_times[time_idx]):
                    ...
                    print(f"Warning: Inconsistent data length for eigenvalue {eigenvalue_index} at time index {time_idx} for width {width}. Skipping.")
                    break # Stop processing for this eigenvalue if data is inconsistent


            if training_times_for_plot: # Only plot if there is data for this eigenvalue
                 pl.plot(training_times_for_plot, f_lambda_t_values, label=f'$\lambda_{{{eigenvalue_index+1}}}$')


        pl.title(f'$f(\lambda, t) = width * (\lambda(t) - \lambda(0))$ vs. Training Time (Width {width})')
        pl.xlabel("Training Time")
        pl.ylabel("$f(\lambda, t)$")
        pl.yscale('log')
        # Add legend if the number of eigenvalues is not too large
        if num_eigenvalues <= 10:
             pl.legend()
        pl.grid(True)
        ...
        plot_filename = os.path.join(plot_eigenvalues_dir, f"eigenvalue_function_width_{width}.png")
        pl.savefig(plot_filename)
        pl.close()
        print(f"Saved eigenvalue function plot for width {width} to {plot_filename}")

print("\nFinished plotting eigenvalue functions.")
...
for data in all_widths_data:
    ...
    # Determine the indices of the largest, smallest, and middle eigenvalues
    if num_eigenvalues > 0:
        sorted_initial_eigenvalue_indices = np.argsort(initial_eigenvalue_spectrum)
        smallest_eigenvalue_index = sorted_initial_eigenvalue_indices[0]
        largest_eigenvalue_index = sorted_initial_eigenvalue_indices[-1]
        middle_eigenvalue_index = sorted_initial_eigenvalue_indices[num_eigenvalues // 2] # Integer division for middle index
        selected_eigenvalue_indices = sorted([smallest_eigenvalue_index, middle_eigenvalue_index, largest_eigenvalue_index]) # Sort for consistent plotting order
    else:
    ...
    eigenvalue_labels = {
        smallest_eigenvalue_index: 'Smallest',
        middle_eigenvalue_index: 'Middle',
        largest_eigenvalue_index: 'Largest'
    }
...
plot_eigenvalues_dir = "plots/eigenvalue_functions"
os.makedirs(plot_eigenvalues_dir, exist_ok=True)
plot_filename = os.path.join(plot_eigenvalues_dir, "combined_eigenvalue_functions.png")
pl.savefig(plot_filename)
pl.close()
