...
# Define the train_network function with 1000 epoch loss recording and time recording
def train_network(model, x_train_split, y_train_split, x_test_split, y_test_split, learning_rate, num_epochs, device):
    ...
    interval_losses = []
    losses_1000_epochs = []
    test_losses_1000_epochs = []
    adjusted_epoch_times = [] # List to store adjusted time at the end of each epoch
    ...
    for epoch in range(num_epochs):
        ...
        adjusted_epoch_times.append(epoch * learning_rate) # Calculate and record adjusted time
        ...
        with torch.no_grad(): # Disable gradient calculations
            ...
            if epoch % 1000 == 0:
                test_losses_1000_epochs.append(test_loss.item()) # Store test loss here
    ...
    return model, interval_losses, train_losses, losses_1000_epochs, test_losses, test_losses_1000_epochs, adjusted_epoch_times



# Update train_model to collect and save 1000-epoch losses and plot loss vs time
def train_model(width):
    device = torch.device("cpu")
    print(f"Using device: {device}")
    print(mp.cpu_count())

    NUM_EPOCHS = 50000 * width
    ...
    all_ensemble_1000_epoch_test_losses = [] # New list for test losses
    all_ensemble_epoch_times = [] # List to store epoch times for each model
    ...
    for i in range(ENSEMBLE_SIZE):
        ...
        trained_model, interval_losses, epoch_losses, losses_1000_epochs, test_losses, test_losses_1000_epochs, adjusted_epoch_times = train_network(model, x_train_split, y_train_split, x_test_split, y_test_split, LEARNING_RATE, NUM_EPOCHS, device)
        ...
        all_ensemble_1000_epoch_test_losses.append(test_losses_1000_epochs) # Append test losses
        all_ensemble_epoch_times.append(adjusted_epoch_times)
    ...
    # Plot loss vs time for each model in the ensemble
    pl.figure(figsize=(10, 6))
    for i in range(ENSEMBLE_SIZE):
        pl.plot(all_ensemble_epoch_times[i], ensemble_epoch_losses[i], label=f"Model {i+1}")
    pl.title(f"Training Loss vs Adjusted Time (Width: {width})")
    pl.xlabel("Adjusted Time (Epochs * Learning Rate)")
    pl.ylabel("MSE")
    pl.yscale("log")
    pl.grid(True)
    pl.legend()
    loss_time_plot_path = os.path.join(plot_dir, f"loss_vs_adjusted_time_width_{width}.png")
    pl.savefig(loss_time_plot_path)
    pl.close()
    ...
    all_ensemble_1000_epoch_test_losses_np = np.array(all_ensemble_1000_epoch_test_losses) # Convert test losses to numpy
    mean_1000_epoch_test_losses = np.mean(all_ensemble_1000_epoch_test_losses_np, axis=0) # Calculate mean test losses
    std_1000_epoch_test_losses = np.std(all_ensemble_1000_epoch_test_losses_np, axis=0) # Calculate std test losses
    ...
    data_to_save = {
        ...
        'mean_1000_epoch_test_losses': mean_1000_epoch_test_losses, # Add test losses to data
        'std_1000_epoch_test_losses': std_1000_epoch_test_losses, # Add test losses to data
        'width': width,
    ...
    return mean_interval_losses, std_interval_losses, loss, std_loss, mean_1000_epoch_losses, std_1000_epoch_losses, mean_1000_epoch_test_losses, std_1000_epoch_test_losses
...
if valid_results:
    ...
    for mean_interval, std_interval, final_mean, final_std, mean_1000_epoch, std_1000_epoch, mean_1000_epoch_test, std_1000_epoch_test, width in valid_results: # Unpack test losses
